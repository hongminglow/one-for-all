...
import type { DemoControl, ApiProp } from "@/storybook/registry/types";
...
export function ComponentDetails(props: { component: ComponentItem }) {
	...
	useEffect(() => {
		...
		const defaultControls = config?.controls ?? null;
	...
	const codeSnippet = useMemo(() => {
		const config = registry[props.component.slug];
		const raw = config?.code ?? doc?.code ?? `See official docs for code:\n${props.component.url}`;
		return raw;
