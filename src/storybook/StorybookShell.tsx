...
export function StorybookShell(props: { children: React.ReactNode }) {
  ...
  const groupedSidebarItems = useMemo(() => {
    ...
    for (const item of COMPONENTS) {
      const haystack =
        `${item.slug} ${item.title} ${item.description} ${item.url}`.toLowerCase();
  ...
  const selectedGroupLabel = useMemo(() => {
    for (const group of groupedSidebarItems) {
      if (group.items.some((x) => x.slug === selected?.slug))
        return group.label;
    }
  ...
  return (
    <div className="min-h-dvh bg-[var(--sb-bg)] text-[var(--sb-text)]">
      <div className="h-[72px] border-b border-[var(--sb-border)] bg-[var(--sb-topbar)]">
        <div className="mx-auto flex h-full w-full max-w-[1440px] items-center px-6">
      ...
      <div className="mx-auto flex w-full max-w-[1440px]">
        <aside className="h-[calc(1120px-72px)] w-[288px] border-r border-[var(--sb-border)] bg-[var(--sb-sidebar)]">
          <div className="px-5 pt-6">
            <div className="text-[12px] font-extrabold tracking-wide text-[var(--sb-text-muted)]">
              {selectedGroupLabel}
            </div>

            <nav className="sb-scroll mt-4 max-h-[860px] space-y-5 overflow-auto pr-2">
                    ...
                    {group.items.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className={isSelected ? "pl-2" : undefined}>
                            {item.title}
                          </span>
                        </Link>
                      );
        ...
        <main className="flex-1 bg-[var(--sb-bg)]">
          <div className="sb-scroll max-h-[calc(1120px-72px)] overflow-auto px-8 py-8">
            {props.children}
          </div>
        </main>
      </div>
      ...
      {isSearchOpen ? (
        <SearchModal open={true} onOpenChange={setIsSearchOpen} />
      ) : null}
    </div>
  );
}
