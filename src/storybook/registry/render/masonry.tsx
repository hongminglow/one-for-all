...
type RenderProps = Partial<ComponentProps<typeof Masonry>>;

const defaultItems: ComponentProps<typeof Masonry>["items"] = [
	{
		id: "1",
...
export default function Render(props: RenderProps) {
	const { items = defaultItems, stagger = 0.05, duration = 0.5, ...rest } = props;

	return (
		<div className="w-full min-h-[600px] bg-[#111] rounded-xl border border-(--sb-border) p-4">
			<Masonry items={items} stagger={stagger} duration={duration} {...rest} />
		</div>
	);
}
