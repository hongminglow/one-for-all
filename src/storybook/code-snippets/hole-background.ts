export const code = `/* eslint-disable @typescript-eslint/no-explicit-any */
...
function HoleBackground({
	...
	const setLines = React.useCallback(() => {
		...
		const currentColor = strokeColor || (isDarkRef.current ? "#525252" : "#e5e5e5");
		...
		// Store rendered lines
		stateRef.current.linesCanvas = offCanvas;
	}, [numberOfLines, strokeColor]);

	// Effect to update lines when theme changes (since React state/props don't change for theme)
	React.useEffect(() => {
		if (!stateRef.current.rect.width) return;

		const updateLinesColor = () => {
			const { width, height } = stateRef.current.rect;
			const offCanvas = document.createElement("canvas");
			offCanvas.width = width;
			offCanvas.height = height;
		...
		// Observer for class changes on html to trigger re-init of lines.
		const observer = new MutationObserver((mutations) => {
			mutations.forEach((mutation) => {
				if (mutation.attributeName === "class") {
					isDarkRef.current = document.documentElement.classList.contains("dark");
	...
	const drawDiscs = React.useCallback(
		(ctx: CanvasRenderingContext2D) => {
			// Use provided strokeColor or adapt to theme
			if (strokeColor) {
				ctx.strokeStyle = strokeColor;
			} else {
				ctx.strokeStyle = isDarkRef.current ? "#525252" : "#e5e5e5";
			}
			ctx.lineWidth = 2;
	...
	// Ref to hold the current tick function
	...
	}, [init, setSize, setDiscs, setLines, setParticles]);

	const defaultFillColor = isDarkRef.current ? "#09090b" : "white";
	...
	return (
		<div
			...
			className={cn(
				...
				'after:content-[""] after:absolute after:z-5 after:top-1/2 after:left-1/2 after:block after:size-full after:bg-[radial-gradient(ellipse_at_50%_75%,#a900ff_20%,transparent_75%)] after:transform-[translate3d(-50%,-50%,0)] after:mix-blend-overlay',
				className,
			)}
			style={{
				...(props.style || {}),
		...
		>
			<div
				className='content-[""] absolute top-1/2 left-1/2 block size-[140%] transform-[translate3d(-50%,-50%,0)]'
				style={{
					background: \`radial-gradient(ellipse at 50% 55%, transparent 10%, \${activeFillColor} 50%)\`,
				}}
			/>
			{children}
			<canvas ref={canvasRef} className="absolute inset-0 block size-full dark:opacity-40 opacity-10" />
			<motion.div
				className={cn(
					...
					"dark:bg-[linear-gradient(20deg,#00f8f1,#ffbd1e20_16.5%,#fe848f_33%,#fe848f20_49.5%,#00f8f1_66%,#00f8f160_85.5%,#ffbd1e_100%)_0_100%/100%_200%] bg-[linear-gradient(20deg,#00f8f1,#ffbd1e40_16.5%,#fe848f_33%,#fe848f40_49.5%,#00f8f1_66%,#00f8f180_85.5%,#ffbd1e_100%)_0_100%/100%_200%]",
				)}
				animate={{ backgroundPosition: "0% 300%" }}
				transition={{ duration: 5, ease: "linear", repeat: Infinity }}
			/>
			<div className="absolute top-0 left-0 z-7 size-full dark:bg-[repeating-linear-gradient(transparent,transparent_1px,#09090b_1px,#09090b_2px)] mix-blend-overlay opacity-50" />
		</div>
	);
}
