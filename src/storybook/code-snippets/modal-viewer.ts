...
const UnifiedLoader: FC<{
	...
	if (ext === "glb" || ext === "gltf") return <GLTFLoaderWrapper url={url}>{children}</GLTFLoaderWrapper>;
	if (ext === "fbx") return <FBXLoaderWrapper url={url}>{children}</FBXLoaderWrapper>;
	if (ext === "obj") return <OBJLoaderWrapper url={url}>{children}</OBJLoaderWrapper>;
