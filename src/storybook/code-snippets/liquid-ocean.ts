...
function OceanMesh({
    ...
    const wireframeMaterial = useMemo(() => new THREE.MeshPhysicalMaterial({ color: accentColor, wireframe: true, transparent: false, opacity: 1 }), [accentColor]);
    const surfaceMaterial = useMemo(() => new THREE.MeshPhysicalMaterial({ color: accentColor, transparent: true, opacity: opacity, wireframe: false }), [accentColor, opacity]);

    return (
        <group rotation={[-90 * Math.PI / 180, 0, 0]}>
            <mesh ref={meshRef} geometry={geometry} material={surfaceMaterial} receiveShadow />
            {showWireframe && <mesh geometry={geometry} material={wireframeMaterial} />}
        </group>
    );
}
...
function Boat({ data, color }: { data: BoatData; color: number }) {
    ...
    return (
        <mesh ref={meshRef} position={data.position} rotation={[0, data.rotationY, 0]} scale={data.scale} castShadow>
...
function BoatGroup({ count, spreadRange, color }: { count: number; spreadRange: number; color: number }) {
    ...
    return <group>{boats.map((boat, i) => <Boat key={i} data={boat} color={color} />)}</group>;
}
...
export function LiquidOcean({
    ...
    return (
        <div ref={containerRef} className={cn("relative w-full h-full min-h-[400px] overflow-hidden bg-black", className)}>
            <Canvas shadows dpr={1} frameloop={isVisible ? "always" : "never"} camera={{ position: [0, 2, 10], fov }} gl={{ antialias: false, alpha: false }} style={{ position: "absolute", inset: 0 }}>
                <SceneContent
