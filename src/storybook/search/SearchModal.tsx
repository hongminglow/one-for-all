...
export function SearchModal(props: { open: boolean; onOpenChange: (next: boolean) => void }) {
	...
	return (
		<div role="dialog" aria-modal="true" aria-label="Search components" className="fixed inset-0 z-[99999]">
			<button
				type="button"
				aria-label="Close"
			...
			<div className="absolute left-1/2 top-24 w-[min(720px,calc(100vw-32px))] -translate-x-1/2 rounded-2xl border border-[var(--sb-border-2)] bg-[var(--sb-card)] shadow-[0_24px_60px_rgba(0,0,0,0.55)]">
				...
				<div className="max-h-[420px] overflow-auto p-2">
					{results.length === 0 ? (
						<div className="px-4 py-10 text-center text-[13px] font-medium text-[var(--sb-text-muted)]">No results</div>
					) : (
							...
							{results.map((item, idx) => (
								...
								>
									<div className="flex items-center justify-between gap-4">
										<div>
											<div className="text-[13px] font-extrabold text-[var(--sb-text-strong)]">{item.title}</div>
										...
										<div className="text-[11px] font-bold text-[var(--sb-text-dim)]">{item.library}</div>
									</div>
								</Link>
							))}
						</div>
				...
				<div className="flex items-center justify-between gap-4 border-t border-[var(--sb-border)] px-5 py-3 text-[12px] font-medium text-[var(--sb-text-muted)]">
					<div>
						Tip: press <span className="font-bold text-[var(--sb-text-strong)]">Ctrl</span> +{" "}
						<span className="font-bold text-[var(--sb-text-strong)]">K</span>
					</div>
					<div>Search matches title + description (supports partial & fuzzy)</div>
				</div>
			</div>
		</div>
	);
