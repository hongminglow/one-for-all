...
const OUTPUT_LIB = path.join(ROOT, "lib", "componentRegistry.generated.ts");
...
function inferCategory(url, tags) {
  const u = String(url || "").toLowerCase();
  const t = tags || [];
  if (u.includes("/text-animations") || t.includes("text-animations"))
    return "Text";
  if (u.includes("/background") || t.includes("backgrounds"))
    return "Background";
  if (u.includes("/animations") || t.includes("animations"))
    return "Animations";
  // Check for components
  if (u.includes("/components") || t.includes("components"))
    return "Components";
...
function parseComponentLines(md) {
  ...
  for (const line of lines) {
    ...
    const slug = slugify(title) || `component-${order}`;

    // Generate tags first so we can use them for category
    const tags = generateTags({ title, slug, url, library });
...
function toLibTs(items) {
  const header = `// This file is auto-generated by scripts/generate-components-data.mjs\n// Do not edit by hand.\n\n`;
  const types = `export type ComponentRegistryItem = {\n  id: string;\n  order: number;\n  slug: string;\n  title: string;\n  description: string;\n  category: string;\n  tags: string[];\n  library: string;\n  docUrl: string;\n};\n\n`;

  const body = `export const COMPONENTS: ComponentRegistryItem[] = ${JSON.stringify(
    items.map((x) => ({
      id: String(x.order),
      order: x.order,
      slug: x.slug,
      title: x.title,
      description: x.description,
      category: inferCategory(x.url, x.tags),
      tags: x.tags,
      library: x.library,
      docUrl: x.url,
    })),
...
await writeFile(OUTPUT_LIB, toLibTs(items), "utf8");
console.log(
  `Generated ${items.length} registry items → ${path.relative(ROOT, OUTPUT_LIB)}`,
);
